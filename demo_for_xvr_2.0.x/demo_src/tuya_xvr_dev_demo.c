...
*/
    ...
    if (0 == g_dev_cnt) {
        ...
        PR_ERR("total cnt  2 bind device error\n");
        return -1;
    }
...
VOID ty_gw_subdev_del_dev(IN CONST CHAR_T *pdevId, IN CONST GW_DELDEV_TYPE type)
{
    ...
    PR_DEBUG("dev_id: %s\n", pdevId);
    tuya_xvr_bind_p2p_auth_info_update(TUYA_XVR_DEV_OPERATE_DEL, pdevId, 0);
#if defined(DEMO_USE_AS_NVR) && (DEMO_USE_AS_NVR==1)
    //need user todo nothing
#else
    ...
    //last step:删除子设备的绑定信息
    tuya_xvr_dev_unbind_cxt_set((char *)pdevId);
#endif
    return;
}
...
VOID ty_gw_subdev_inform_dev(IN CONST CHAR_T *dev_id, IN CONST OPERATE_RET op_ret)
{
    ...
    //step：设备绑定后，需要调用这个函数(否则APP不会更新P2P密码)，另外这样在拉流的时候，就不会有请求密码的过程。把请求密码的过程放在这里，可以节约拉流的网络开销
    tuya_xvr_bind_p2p_auth_info_update(TUYA_XVR_DEV_OPERATE_UPDATE_PASSWORD, dev_id, op_ret);// NOTICE: update sub dev
    //step：这个函数用户通知SDK子设备绑定结果。务必调用
    tuya_xvr_dev_bind_result_cb((CHAR_T*)dev_id, op_ret);
#if defined(DEMO_USE_AS_NVR) && (DEMO_USE_AS_NVR==1)
        //need user todo nothing
#else
    if (OPRT_OK == op_ret){
