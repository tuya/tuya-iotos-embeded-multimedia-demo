...
STATIC VOID __TUYA_APP_ss_pb_event_cb(IN CHAR_T * dev_id,IN UINT_T pb_idx, IN SS_PB_EVENT_E pb_event, IN PVOID_T args)
{
    PR_DEBUG("ss pb rev event: %u %d", pb_idx, pb_event);
    if(pb_event == SS_PB_FINISH)
    {
       // tuya_xvr_playback_finish_status_send(NULL,pb_idx);
        tuya_ipc_playback_send_finish_with_channel(0,pb_idx);//jtodo zxq
    }
}

STATIC VOID __TUYA_APP_ss_pb_get_video_cb(IN CHAR_T * devId,IN UINT_T pb_idx, IN CONST MEDIA_FRAME_S *p_frame)
{
...
STATIC VOID __TUYA_APP_ss_pb_get_audio_cb(IN CHAR_T *devId, IN UINT_T pb_idx, IN CONST MEDIA_FRAME_S *p_frame)
{
    TRANSFER_AUDIO_FRAME_S audio_frame = {0};
    __TUYA_APP_media_frame_TO_trans_audio(p_frame, &audio_frame);
    tuya_ipc_playback_send_audio_frame_with_channel(0, pb_idx, &audio_frame);
}
...
STATIC INT_T __TUYA_APP_event_cb(IN CONST TRANSFER_EVENT_E event, IN CONST PVOID_T args)
{
...

    {
        ...
        OPERATE_RET ret = tuya_ipc_pb_query_by_day(pquery->subdid,pquery->channel,pquery->year, pquery->month, pquery->day, &p_day_ts);
        if (OPRT_OK != ret)
        {
            PR_ERR("pb_ts chn[%d] query by day: %d-%d-%d Fail\n", pquery->channel,pquery->year, pquery->month, pquery->day);
            break;
    ...
    case TRANS_PLAYBACK_START_TS_GW:
    {
        ...
        ret = tuya_ipc_pb_start(pParam->subdid, pParam->channel, __TUYA_APP_ss_pb_event_cb, __TUYA_APP_ss_pb_get_video_cb, __TUYA_APP_ss_pb_get_audio_cb);
        if (0 != ret){
            printf("%s %d pb_start failed\n",__FUNCTION__,__LINE__);
           // tuya_ipc_playback_send_finish_with_channel(pParam->subdid,pParam->channel);//zxq
        }else{
            if (0 != tuya_ipc_pb_seek(pParam->subdid, pParam->channel, &pb_file_info, pParam->playTime)){
                printf("%s %d pb_seek failed\n",__FUNCTION__,__LINE__);
                tuya_ipc_playback_send_finish_with_channel(pParam->channel,pParam->idx);
            }
        }
#endif
        break;
...
#if defined(__USER_DO_NOT_OPEN__)&&(__USER_DO_NOT_OPEN__==1)
        tuya_ipc_ss_pb_set_status(pParam->subdid,pParam->channel, SS_PB_PAUSE);
#endif
        break;
    }
    ...
    case TRANS_PLAYBACK_MUTE_GW:
    {
        ...
        PR_DEBUG("PB idx:%d mute\n", pParam->channel);
                        ...
                        //custom do it self
#else
        tuya_ipc_ss_pb_set_status(pParam->subdid,pParam->channel, SS_PB_MUTE);
#endif
        break;
    }
    case TRANS_PLAYBACK_UNMUTE_GW:
    {
        ...
        tuya_ipc_ss_pb_set_status(pParam->subdid, pParam->channel, SS_PB_UN_MUTE);
#endif
        break;
    }
    case TRANS_PLAYBACK_STOP_GW:
    {
        ...
        tuya_ipc_pb_stop((CHAR_T *)pParam->subdid, pParam->channel);
#endif
        break;
    }
