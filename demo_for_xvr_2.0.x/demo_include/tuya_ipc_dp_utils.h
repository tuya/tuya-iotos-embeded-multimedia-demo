...
/***DP for NVR/BaseStation***/
#define TUYA_DP_XVR_SUPPORT_STORAGE            102        /* VALUE 类型，按位赋值 bit0表示sd卡 bit1表示HDD；1表示检测到  0表示未检测到*/
#define TUYA_DP_XVR_STOR_VOLUME                  103        /* STOR容量,STR类型,"总容量|当前使用量|剩余容量",单位：kb */
#define TUYA_DP_XVR_STOR_STATUS                  104        /* STOR状态,VALUE类型,1-正常,2-异常,3-空间不足,4-正在格式化,5-无SD卡 */
#define TUYA_DP_XVR_STOR_FORMAT                  105         /* 格式化STOR,BOOL类型 */
#define TUYA_DP_XVR_STOR_UNMOUNT                 106         /* 卸载STOR,BOOL类型 */
#define TUYA_DP_XVR_STOR_FORMAT_STATUS           107         /* 格式化STOR状态,VALUE类型,-2000:SD卡正在格式化,-2001:SD卡格式化异常,-2002:无SD卡,-2003:SD卡错误.正数为格式化进度 */
#define TUYA_DP_XVR_SET_CUR_STORAGE             113         /* 设置当前储存设备 VALUE 类型，按位赋值 bit0表示sd卡 bit1表示HDD；1启用  0关闭    ， 不可同时启动多个设备*/

/**dp for xvv main device*/
#define TUYA_DP_LED                        101        /* BOOL 设备指示灯    0 关闭  1 开启*/
#define TUYA_DP_SUPPORT_STORAGE            102        /* VALUE 类型，按位赋值 bit0表示sd卡 bit1表示HDD；1表示检测到  0表示未检测到*/
#define TUYA_DP_STOR_VOLUME                  103        /* STOR容量,STR类型,"总容量|当前使用量|剩余容量",单位：kb */
#define TUYA_DP_STOR_STATUS                  104        /* STOR状态,VALUE类型,1-正常,2-异常,3-空间不足,4-正在格式化,5-无SD卡 */
#define TUYA_DP_STOR_FORMAT                  105         /* 格式化STOR,BOOL类型 */
#define TUYA_DP_STOR_UNMOUNT                 106         /* 卸载STOR,BOOL类型 */
#define TUYA_DP_STOR_FORMAT_STATUS           107         /* 格式化STOR状态,VALUE类型,-2000:SD卡正在格式化,-2001:SD卡格式化异常,-2002:无SD卡,-2003:SD卡错误.正数为格式化进度 */

#define TUYA_DP_SET_CUR_STORAGE             113         /* 设置当前储存设备 VALUE 类型，按位赋值 bit0表示sd卡 bit1表示HDD；1启用  0关闭    ， 不可同时启动多个设备*/
#define TUYA_DP_SIREN_SWITCH                114         /* siren告警开关，BOOL类型*/

#define TUYA_SUB_DEV_WAKEUP                 149         /* 子设备唤醒，BOOL类型*/

#define SD_ENABLE_VALUE     (1)
#define HDD_ENABLE_VALUE     (2)

#define TY_SUB_DEV_SLEEPTIME_PID 223


/***dp for xvr sub device***/
