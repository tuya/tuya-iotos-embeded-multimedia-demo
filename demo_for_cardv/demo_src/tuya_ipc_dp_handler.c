...
#define tuya_cfg_path "/tmp/tuya.cfgs"
...
STATIC VOID __tuya_app_write_INT(CHAR_T* key, INT_T value)
{
    ...
    snprintf(tmp_cmd, 128, "mkdir -p %s;echo %d > %s/%s", tuya_cfg_path, value, tuya_cfg_path, key);
    printf("write int exc: %s \r\n", tmp_cmd);
    system(tmp_cmd);
}

INT_T __tuya_app_read_INT(CHAR_T* key)
{
    //TODO

    CHAR_T tmp_file[64] = { 0 };
    snprintf(tmp_file, 64, "cat %s/%s", tuya_cfg_path, key);
...
STATIC VOID __tuya_app_write_STR(CHAR_T* key, CHAR_T* value)
{
    ...
    snprintf(tmp_cmd, 256, "echo %s > %s/%s", value, tuya_cfg_path, key);
...
STATIC CHAR_T response[64] = { 0 };
CHAR_T* IPC_APP_get_ap_mode(VOID)
{
    CHAR_T ap_ssid[32] = { 0 };
    CHAR_T ap_pw[64] = { 0 };
    int cur_mode = WWM_STATION;
    NW_MAC_S mac = { { 0 } };
    user_wifi_get_work_mode(&cur_mode);
    INT_T is_ap = 0;

    __tuya_app_read_STR("tuya_ap_ssid", ap_ssid, sizeof(ap_ssid));
    if (ap_ssid[0] == 0) {
        strncpy(ap_ssid, "ipc_tuya", 32);
    }

    OPERATE_RET op_ret = user_wifi_get_mac(&mac);
    // strncpy(ap_ssid, "ipc_tuya", 32);
    ...
    __tuya_app_read_STR("tuya_ap_passwd", ap_pw, sizeof(ap_pw));

    snprintf(response, 128, "{\\\"is_ap\\\":%d,\\\"ap_ssid\\\":\\\"%s\\\",\\\"password\\\":\\\"%s\\\"}", is_ap, ap_ssid, ap_pw);
    // printf("response:%s\n", response);
    return response;
}
CHAR_T* IPC_APP_get_ap_mode_in_db(VOID)
{
    CHAR_T ap_ssid[32] = { 0 };
    CHAR_T ap_pw[64] = { 0 };
    NW_MAC_S mac = { { 0 } };
    ...
    if(ap_ssid[0] == 0)
    {
        strncpy(ap_ssid, "TUYA_IPC", 32); 
    }
    
    OPERATE_RET op_ret = user_wifi_get_mac(&mac);
    if(OPRT_OK != op_ret) {
        strncpy(ap_ssid, "TUYA_IPC_AP", 32);        
    }
    else
    {
        snprintf(ap_ssid+strlen(ap_ssid), 32-strlen(ap_ssid), "-%02X%02X", mac.mac[4],mac.mac[5]);   
    }
#endif
    strncpy(ap_ssid, "ipc_tuya", 32);

    //{ is_ap: 1, ap_ssid: "xxxx"}
    is_ap = __tuya_app_read_INT("tuya_is_ap");
    __tuya_app_read_STR("tuya_ap_passwd", ap_pw, sizeof(ap_pw));
    snprintf(response, 128, "{\\\"is_ap\\\":%d,\\\"ap_ssid\\\":\\\"%s\\\",\\\"password\\\":\\\"%s\\\"}", is_ap, ap_ssid, ap_pw);
    return response;
}
...
VOID* __ap_change_thread(void* arg)
{
    ...
    INT_T ap_onoff = 0;
    sleep(1);
    ap_onoff = __tuya_app_read_INT("tuya_is_ap");
    printf("get ap onfff %d\n", ap_onoff);
    if (ap_onoff) {
        printf("##sh /mnt/extsd/ap_mode.sh start");
        system("sh /mnt/extsd/ap_mode.sh start");
        usr_set_net_ap();
    } else {
        if (cur_mode == WWM_SOFTAP) {
            printf("##sh /mnt/extsd/ap_mode.sh stop");
            system("sh /mnt/extsd/ap_mode.sh stop");
            usr_set_net_wire();
        } else {
            printf("## not ap mode,do nothing  ");
    ...
    if(ap_onoff)
    {
        ...
        __tuya_app_read_STR("tuya_ap_ssid", ap_cfg.ssid, sizeof(ap_cfg.ssid));
        __tuya_app_read_STR("tuya_ap_passwd", ap_cfg.passwd, sizeof(ap_cfg.passwd));
        
        OPERATE_RET op_ret = user_wifi_get_mac(&mac);
        if(OPRT_OK != op_ret)
        {
            strncpy((char *)(ap_cfg.ssid), "TUYA_IPC_AP", 32); 
        }
        else
        {
            if(ap_cfg.ssid[0] == 0)
            {
                strncpy((char *)(ap_cfg.ssid), "TUYA_IPC", 32); 
            }
            snprintf((char *)(ap_cfg.ssid)+strlen((char *)(ap_cfg.ssid)), 32-strlen((char *)(ap_cfg.ssid)), "-%02X%02X", mac.mac[4],mac.mac[5]);
        }
...
VOID change_ap_process()
{
    ...
    if (ret != 0) {
        printf("ap_change_thread ,create fail! %s ret:%d\n", strerror(errno), ret);
    }
...
INT_T IPC_APP_set_ap_mode(IN cJSON* p_ap_info)
{
    ...
    __tuya_app_write_INT("tuya_is_ap", ap_onoff);
...
BOOL_T get_custom_dp_sync_time_status()
{
    return time_sync;
}
void set_custom_dp_sync_time_status(BOOL_T status)
{
    time_sync = status;
}
...
void ty_time_set_timezone(char* tzfile, time_t time_utc, int timezone)
{
    unsigned int tzValue = 0;
    ...
    tzValue = (timezone > 0) ? timezone : (-timezone);
    ...
    if (timezone > 0) {
        snprintf(cmd, sizeof(cmd), "echo GMT-%02d:%02d:%02d > %s", hour, min, sec, tzfile);
    } else {
        snprintf(cmd, sizeof(cmd), "echo GMT+%02d:%02d:%02d > %s", hour, min, sec, tzfile);
    }
...
// 设置系统时区
void tycam_dev_set_time_zone(time_t time_utc, int timezone)
{
    printf("set timezone :%d\n", timezone);
    return ty_time_set_timezone("/var/conf/TZ", time_utc, timezone);
}
...
VOID IPC_APP_set_time_zone(IN CHAR_T* p_time_info)
{
    ...
    printf("%s %d recv timeZONE sync from app:%s \r\n", __FUNCTION__, __LINE__, p_time_info);
    if (timezone_sync == TRUE) {
        return;
    }

    extern OPERATE_RET uni_set_time_zone(IN CONST CHAR_T * time_zone);
    uni_set_time_zone(p_time_info);
    INT_T time_zone;
    TIME_T time_utc;

    tuya_ipc_get_service_time(&time_utc, &time_zone);
    printf("Get Server Time Success: %d\n", time_zone);
    tycam_dev_set_time_zone(time_utc, time_zone);
