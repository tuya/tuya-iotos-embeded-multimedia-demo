...
#define TUYA_DP_AP_MODE                    231         /* 查询设备当前设备是否处于 ap 模式，布尔类型，APP 下发 NULL 表示查询，设备返回状态值， 1 为正处于 AP 模式，0 为非 AP 模式。*/
/* 
   用于下发ssid 和密码，开启/关闭 AP 模式，
   数据格式：{ ap_enable : 1, ap_ssid : xxxx, ap_pwd : xxx }。
   设备返回数据格式：{ errcode : 0 }.  
   ap_enable 为必须字段，布尔类型，1 为开启 AP 模式，此时 ap_ssid 为必须字段， ap_pwd 为可选字段，没有则不需要密码。0 为关闭 AP 模式，ap_ssid 和 ap_pwd 都为可选
*/
