...
#define DEMO_FAKE_VIDEO_MAX_NUM (16)
...
void *thread_live_audio(void *arg)
{
    ...
    if(aFp == NULL)
    {
        printf("can't read live audio files %s\n",fullpath);
        pthread_exit(0);
    }
...
int TUYA_APP_fake_video_stop(char * devId)
{
    ...
    for (i = 0; i < DEMO_FAKE_VIDEO_MAX_NUM; i++){
        if (1 == s_fake_video[i].valid){
            if(strcmp(devId,s_fake_video[i].devId) == 0) {
